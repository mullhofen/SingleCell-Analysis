...
seurat$celltype <- setNames(rep(c("RPC","BC (RGC?)","HC","AC", "BC", "Cone", "Rod", "MG", "RPE"), 
                                c(1,1,1,1,2, 1, 1, 2, 1 )),
                            c(7,8,6,11, c(9,10), 1, 0, c(3,4), 5))[as.character(seurat$wsnn_res.0.2)]

p_cluster_celltypes <- UMAPPlot(seurat, group.by = "celltype", label=T) & NoAxes()

p_cluster_celltypes

#all clusters seperately
...
motif_enrichment_RGC <- FindMotifs(seurat,
                                     features = top_peaks_ct$feature[top_peaks_ct$group == "BC (RGC?)"],
                                     background = peaks_matched) %>%
  mutate(symbol = setNames(ifelse(is.na(df_pfm$symbol), df_pfm$name, df_pfm$symbol), df_pfm$id)[motif]) %>%
  mutate(padj = p.adjust(pvalue, method="BH"))
enriched_motif_RGC <- motif_enrichment_RGC %>%
  dplyr::filter(padj < 0.01 & fold.enrichment > 3) %>%
  top_n(-4, wt = padj)

#plot top 4 overrepresented TF binding motifs for cell types
p_BP <- MotifPlot(seurat, motifs = enriched_motif_BP$motif[1:4], ncol = 4)
p_cone <- MotifPlot(seurat, motifs = enriched_motif_Cone$motif[1:4], ncol=4)
p_BP / p_cone

p_HC <- MotifPlot(seurat, motifs = enriched_motif_HC$motif[1:4], ncol = 4)
p_MG <- MotifPlot(seurat, motifs = enriched_motif_MG$motif[1:4], ncol=4)
p_HC / p_MG

p_RPE <- MotifPlot(seurat, motifs = enriched_motif_RPE$motif[1:4], ncol = 4)
p_RP <- MotifPlot(seurat, motifs = enriched_motif_RP$motif[1:4], ncol=4)
p_RPE / p_RP

p_Rod <- MotifPlot(seurat, motifs = enriched_motif_Rod$motif[1:4], ncol = 4)
p_AC <- MotifPlot(seurat, motifs = enriched_motif_AC$motif[1:4], ncol = 4)
p_Rod / p_AC

p_RGC <- MotifPlot(seurat, motifs = enriched_motif_RGC$motif[1:4], ncol = 4)
p_RGC
```
```{r}
...
#write a for loop
# Looping through the variable last part
for (i in c("RP", "RGC", "HC", "AC", "BP", "Cone", "Rod", "MG", "RPE")) {
  # Constructing the variable name using paste0
  temp_name <- get(paste0("enriched_motif_", i))
  temp_name$motif.name
  }
