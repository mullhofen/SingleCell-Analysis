...
p_cluster_celltypes <- UMAPPlot(seurat, group.by = "celltype", label=T) & NoAxes()
...
#top2 markers for RNA cell type
top_markers_ct %>% group_by(group) %>% top_n(1, wt = auc) 
...
p_rod <- CoveragePlot(seurat,
                   ...
                   extend.upstream = 1000,
                   extend.downstream = 1000)
patchwork::wrap_plots(p_BP, p_cone, ncol = 1)
...
#bipolar cell cytoplasm
p_BP <- CoveragePlot(seurat,
                   region = "chr2-219509052-219517235",
                   group.by = "celltype",
                   extend.upstream = 1000,
                   extend.downstream = 1000)
...
motif_enrichment_BP <- FindMotifs(seurat,
                                    features = top_peaks_ct$feature[top_peaks_ct$group == "bipolar cell cytoplasm"],
...
motif_enrichment_HC <- FindMotifs(seurat,
                                    features = top_peaks_ct$feature[top_peaks_ct$group == "horizontal cells"],
                                    background = peaks_matched) %>%
  mutate(symbol = setNames(ifelse(is.na(df_pfm$symbol), df_pfm$name, df_pfm$symbol), df_pfm$id)[motif]) %>%
  mutate(padj = p.adjust(pvalue, method="BH"))
...
motif_enrichment_MG <- FindMotifs(seurat,
                                    features = top_peaks_ct$feature[top_peaks_ct$group == "Müller glia"],
                                    background = peaks_matched) %>%
  mutate(symbol = setNames(ifelse(is.na(df_pfm$symbol), df_pfm$name, df_pfm$symbol), df_pfm$id)[motif]) %>%
  mutate(padj = p.adjust(pvalue, method="BH"))
...
motif_enrichment_RP <- FindMotifs(seurat,
                                    features = top_peaks_ct$feature[top_peaks_ct$group == "retinal progenitor cells"],
                                    background = peaks_matched) %>%
  mutate(symbol = setNames(ifelse(is.na(df_pfm$symbol), df_pfm$name, df_pfm$symbol), df_pfm$id)[motif]) %>%
  mutate(padj = p.adjust(pvalue, method="BH"))
...
p_BP <- MotifPlot(seurat, motifs = enriched_motif_BP$motif[1:6], ncol = 4)
p_cone <- MotifPlot(seurat, motifs = enriched_motif_Cone$motif[1:6], ncol=4)
p_BP / p_cone

p_HC <- MotifPlot(seurat, motifs = enriched_motif_HC$motif[1:6], ncol = 4)
p_MG <- MotifPlot(seurat, motifs = enriched_motif_MG$motif[1:6], ncol=4)
p_HC / p_MG

p_RPE <- MotifPlot(seurat, motifs = enriched_motif_RPE$motif[1:6], ncol = 4)
p_RP <- MotifPlot(seurat, motifs = enriched_motif_RP$motif[1:6], ncol=4)
p_RPE / p_RP

p_Rod <- MotifPlot(seurat, motifs = enriched_motif_Rod$motif[1:6], ncol = 4)
p_Rod
```
```{r}
...
p_BP <- FeaturePlot(seurat,
                  c("NEUROD1","NEUROG(var.2)","Ptf1a","TAL1::TCF3", "Ascl2", "BHLHA15(var.2)"),
                  reduction="umap",
                  order=T,
                  ncol=4) & NoAxes() & NoLegend()
p_cone <- FeaturePlot(seurat,
                  c("Crx","GSC2","OTX2","GSC", "Dmbx1", "PITX3"),
                  reduction="umap",
                  order=T,
                  ncol=4) & NoAxes() & NoLegend()
p_HC <- FeaturePlot(seurat,
                  c("EGR2","NR2C2","KLF17","Znf281", "MZF1(var.2)", "ZBTB14"),
                  reduction="umap",
                  order=T,
                  ncol=4) & NoAxes() & NoLegend()
p_MG <- FeaturePlot(seurat,
                  c("MZF1(var.2)","Sox6","Znf281","SOX4", "PLAG1", "NFIB"),
                  reduction="umap",
                  order=T,
                  ncol=4) & NoAxes() & NoLegend()
p_RPE <- FeaturePlot(seurat,
                  c("NFIB","Arnt","NFIX(var.2)","ZNF740", "MZF1(var.2)", "TFAP2A"),
                  reduction="umap",
                  order=T,
                  ncol=4) & NoAxes() & NoLegend()
p_RP <- FeaturePlot(seurat,
                  c("MZF1(var.2)","PLAG1","Znf281","STAT1", "ZNF740", "Stat4"),
                  reduction="umap",
                  order=T,
                  ncol=4) & NoAxes() & NoLegend()
p_Rod <- FeaturePlot(seurat,
                  c("GSC2","GSC","Dmbx1","Crx", "PITX3","NEUROG(var.2)") ,
                  reduction="umap",
                  order=T,
                  ncol=4) & NoAxes() & NoLegend()
...
tfs <- read.table("/Users/moritzullhofen/Desktop/ETH/SS23/AA Single Cell Technologies/DryLab/#2 More advanced single cell analyses/AA 3 Analysis of single-cell multiome data (RNA & ATAC)/Data/Homo_sapiens_TF.txt", sep="\t", header=T)
